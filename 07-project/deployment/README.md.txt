...
(base) ubuntu@ip-172-31-9-143:~$ source /home/ubuntu/.local/share/virtualenvs/deployment-r6REQ1xB/bin/activate
...
// Fully deployed Prefect workflow for batch scoring is created. Refer to score_deploy.py.
// It's a cron job scheduled to run at 3AM on 2nd of every month.
...
//Now, run the docker container to start Flask App on gunicorn server 
...
2. In another terminal, run test.py to get the prediction from this service. Following are the examples.
	...
	python test.py 23A 43B electric_bike casual 1.4
	python test.py 23A 43B docked_bike member 3.6
//Arguments are 'start_station_id', 'end_station_id', 'rideable_type', 'member_casual', and 'trip_distance'.
